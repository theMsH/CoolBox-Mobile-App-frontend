...
interface ConsumptionApi {
    ...
    suspend fun getDailyConsumptionsDataFromLastSevenDaysByHours(
    ...
    suspend fun getWeeklyConsumptionsData(
        @Path("date") date: String
    ): ConsumptionStatsResponse

    // Hae tunneittainen sisälämpötilatiedot annetun päivämäärän perusteella
    @GET("measurement/temperature/indoor/avg/hourly/{date}")
    suspend fun getHourlyTemperaturesData(
        @Path("date") date: String
